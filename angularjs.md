# Angular & Django Integration Guide

This document outlines the step-by-step process of creating a full-stack application with a Django REST API backend and an Angular frontend.

## Part 1: Django Backend Setup (rest_api)

### 1. Project and App Creation
- **Initialize Project**: `django-admin startproject rest_api`
- **Create App**: `python manage.py startapp users`
- **Install Dependencies**: `pip install djangorestframework django-cors-headers`

### 2. Model Definition (`users/models.py`)
Defined a `User` model with fields: `name`, `username`, `email`, `password`, and `phone`.

### 3. API Serialization (`users/serializers.py`)
Created a `UserSerializer` to convert model instances into JSON format for the API.

### 4. ViewSets and Logic (`users/views.py`)
- Implemented `UserViewSet` for standard CRUD operations.
- Added a custom `@action` called `login` to handle `POST` requests for authentication.

### 5. Routing (`rest_api/urls.py`)
Used DRF's `DefaultRouter` to automatically generate URLs for the `users` endpoint, including the custom login action (`/api/users/login/`).

---

## Part 2: Angular Frontend Setup (my-angular-app)
